...
class SnipCompactorTest {
    ...
    @Test
    void compact_withLongJsonToolOutput_preservesJsonIntact() {
        String longJson = """
                {
        ...
        // JSON should be preserved intact - no truncation notice for JSON
        assertEquals(longJson, output, "Long JSON should be preserved intact for AutoCompact to handle");
    ...
    @Test
    void compact_withLongNonJsonToolOutput_truncatesWithSuffix() {
        // Non-JSON multi-line output should be truncated
        String longOutput = "line1\nline2\nline3\n" + "x".repeat(200);
        List<Message> messages = List.of(
                new AssistantMessage(longOutput)
        );
        ...
        assertTrue(output.length() < longOutput.length(), "Should be shorter than original");
    }
