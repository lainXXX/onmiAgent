...
这是工业级 Agent（如 Anthropic Computer Use、Open Hands）采用的**最佳实践**：

| 层级 | 名称 | 作用 | 实现方式 |
|------|------|------|----------|
| **第一层** | System Prompt (预防) | 告诉 Agent"你在 Bash 环境"，明确路径/命令转换规则 | 更新 `agent_system_prompt.md` |
| **第二层** | 环境探测 (感知) | Agent 启动时确认环境类型（WSL/Git Bash/原生 Linux） | 集成到工具层或 Prompt |
| **第三层** | 工具错误反馈 (兜底) | 命令失败时返回结构化错误报告 | BashErrorClassifier |

```
┌─────────────────────────────────────────────────┐
│  第一层：System Prompt 预防                       │
│  "你运行在 Bash 环境，Windows 路径需转换"         │
└─────────────────────────────────────────────────┘
                    ↓ 边缘情况
┌─────────────────────────────────────────────────┐
│  第二层：环境探测 + 第三层：错误反馈              │
...
| 层级 | 组件 | 职责 | 不做 |
|------|------|------|------|
| **第一层** | `agent_system_prompt.md` | 环境认知锁定、路径/命令转换规则 | 不执行命令 |
| **第二层** | 探测命令 (`pwd`/`uname`) | 确认当前环境类型 | 不修改任何文件 |
| **第三层** | `BashToolConfig` | 检测错误、返回清晰错误信息、纠正方向 | 不自动翻译命令 |
...
| Phase | 内容 | 产出 |
|-------|------|------|
| **Phase 0** | System Prompt 更新 | 环境感知指导 |
| **Phase 1** | `OsDetector` + `CommandStyleDetector` | 系统检测 |
| **Phase 2** | `BashErrorClassifier` | 错误分类器 |
| **Phase 3** | `ErrorReportBuilder` | 报告构建器 |
| **Phase 4** | `BashToolConfig` 集成 | 整合各组件 |
